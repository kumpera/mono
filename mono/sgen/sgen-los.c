...
get_los_section_memory (size_t size)
{
 ...
 retry:
	...
	section = (LOSSection *)sgen_alloc_os_memory_aligned (LOS_SECTION_SIZE, LOS_SECTION_SIZE, (SgenAllocFlags)(SGEN_ALLOC_HEAP | SGEN_ALLOC_ACTIVATE | SGEN_ALLOC_NON_FATAL), "los-section");
...
sgen_los_alloc_large_inner (GCVTable vtable, size_t size)
{
...
#ifdef LOS_DUMMY
	if (!los_segment)
		los_segment = sgen_alloc_os_memory (LOS_SEGMENT_SIZE, SGEN_ALLOC_HEAP | SGEN_ALLOC_ACTIVATE, "los-dummy-segment");
	...
	if (size > LOS_SECTION_OBJECT_LIMIT) {
		...
		if (sgen_memgov_try_alloc_space (alloc_size, SPACE_LOS)) {
			obj = (LOSObject *)sgen_alloc_os_memory (alloc_size, (SgenAllocFlags)(SGEN_ALLOC_HEAP | SGEN_ALLOC_ACTIVATE | SGEN_ALLOC_NON_FATAL), "los-object");
			if (obj) {
