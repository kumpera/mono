...
emit_object_to_ptr_conv (MonoMethodBuilder *mb, MonoType *type, MonoMarshalConv conv, MonoMarshalSpec *mspec)
{
	...
	case MONO_MARSHAL_CONV_STR_TBSTR: {
		...
		mono_mb_emit_ldloc (mb, 0);
		mono_mb_emit_byte (mb, CEE_LDIND_REF);
		mono_mb_emit_icall_by_name (mb, conv_to_icall (conv, &stind_op));
	...
	case MONO_MARSHAL_CONV_STR_BYVALWSTR: {
		...
		mono_mb_emit_icon (mb, mspec->data.array_data.num_elem);
		mono_mb_emit_icall_by_name (mb, conv_to_icall (conv, NULL));
		break;
	}
	case MONO_MARSHAL_CONV_ARRAY_BYVALARRAY: {
		MonoClass *eklass = NULL;
...
static void
emit_thread_interrupt_checkpoint_call (MonoMethodBuilder *mb, const char *checkpoint_func_name)
{
	int pos_noabort, pos_noex;

	// printf (">mono_thread_force_interruption_checkpoint_noraise cp func %p\n", checkpoint_func_name);
	...
	mono_mb_emit_icall_by_name (mb, checkpoint_func_name);
					 ...
					 MarshalAction action)
{
	...
	case MARSHAL_ACTION_MANAGED_CONV_RESULT:
		if (!strcmp (conv_to_icall (conv, NULL), "mono_marshal_string_to_utf16"))
		...
		*conv_arg_type = &mono_defaults.int_class->byval_arg;
		...
		if (klass->delegate) {
			if (t->byref) {
				...
				mono_mb_emit_ldarg (mb, argnum);
				mono_mb_emit_icall_by_name (mb, conv_to_icall (MONO_MARSHAL_CONV_DEL_FTN, NULL));
				mono_mb_emit_stloc (mb, conv_arg);
			}
	...
	case MARSHAL_ACTION_CONV_OUT:
		...
		if (klass->delegate) {
			if (t->byref) {
				...
				mono_mb_emit_op (mb, CEE_MONO_CLASSCONST, klass);
				mono_mb_emit_ldloc (mb, conv_arg);
				mono_mb_emit_icall_by_name (mb, conv_to_icall (MONO_MARSHAL_CONV_FTN_DEL, NULL));
				mono_mb_emit_byte (mb, CEE_STIND_REF);
			}
			break;
		}
	...
	case MARSHAL_ACTION_MANAGED_CONV_OUT:
		if (klass->delegate) {
			if (t->byref) {
				...
				mono_mb_emit_ldarg (mb, argnum);
				mono_mb_emit_ldloc (mb, conv_arg);
				mono_mb_emit_icall_by_name (mb, conv_to_icall (MONO_MARSHAL_CONV_DEL_FTN, &stind_op));
				mono_mb_emit_byte (mb, stind_op);
				break;
			}
		}
