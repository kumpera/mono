...
encode_type (MonoDynamicImage *assembly, MonoType *type, SigBuffer *buf)
{
	...
	case MONO_TYPE_CLASS: {
		...
		if (mono_class_has_generic_container (k)) {
...
type_get_qualified_name (MonoType *type, MonoAssembly *ass) {
	...
	if (assembly_is_dynamic (ta) || (ta == ass)) {
		if (mono_class_is_ginst (klass) || mono_class_has_generic_container (klass))
...
field_encode_signature (MonoDynamicImage *assembly, MonoReflectionFieldBuilder *fb)
{
	...
	if (typespec) {
		...
		gclass = mono_metadata_lookup_generic_class (class, mono_class_get_generic_container (class)->context.class_inst, TRUE);
		encode_generic_class (assembly, gclass, &buf);
	} else {
		encode_type (assembly, type, &buf);
	}
...
create_typespec (MonoDynamicImage *assembly, MonoType *type)
{
	...
	case MONO_TYPE_VALUETYPE: {
		MonoClass *k = mono_class_from_mono_type (type);
		if (!k || !mono_class_has_generic_container (k)) {
			sigbuffer_free (&buf);
			return 0;
		}
		encode_type (assembly, type, &buf);
...
{
	return (mono_class_is_ginst (field->parent) && mono_class_get_generic_class (field->parent)->is_dynamic && ((MonoDynamicGenericClass*)mono_class_get_generic_class (field->parent))->fields);
}
...
get_field_on_inst_generic_type (MonoClassField *field)
{
	...
	dgclass = (MonoDynamicGenericClass*)mono_class_get_generic_class (field->parent);
	...
	class = field->parent;
	gtd = mono_class_get_generic_class (class)->container_class;
...
create_generic_typespec (MonoDynamicImage *assembly, MonoReflectionTypeBuilder *tb)
{
	...
	sigbuffer_add_value (&buf, MONO_TYPE_GENERICINST);
	g_assert (mono_class_has_generic_container (klass));
	sigbuffer_add_value (&buf, klass->byval_arg.type);
	sigbuffer_add_value (&buf, mono_image_typedef_or_ref_full (assembly, &klass->byval_arg, FALSE));
	...
	guint32 module_index, guint32 parent_index, MonoDynamicImage *assembly)
{
	...
	/* Emit nested types */
	if (mono_class_get_ext (klass) && mono_class_get_ext (klass)->nested_classes) {
		GList *tmp;

		for (tmp = mono_class_get_ext (klass)->nested_classes; tmp; tmp = tmp->next)
	...
	for (i = 0; i < ilgen->num_token_fixups; ++i) {
		...
		case MONO_TABLE_MEMBERREF:
			if (!strcmp (iltoken->member->vtable->klass->name, "MonoArrayMethod")) {
				...
				MonoMethod *m = ((MonoReflectionMethod*)iltoken->member)->method;
				g_assert (mono_class_is_ginst (m->klass) || mono_class_has_generic_container (m->klass));
				continue;
						 ...
						 gboolean create_open_instance, gboolean register_token)
{
	...
	if (strcmp (klass->name, "MethodBuilder") == 0) {
		...
		token = mono_metadata_token_from_dor (
			mono_image_typedef_or_ref_full (assembly, type, !mono_class_has_generic_container (mc) || create_open_instance));
...
mono_reflection_get_token (MonoObject *obj)
{
	...
	if (strcmp (klass->name, "MethodBuilder") == 0) {
		...
		if (is_field_on_inst (f->field)) {
			MonoDynamicGenericClass *dgclass = (MonoDynamicGenericClass*)mono_class_get_generic_class (f->field->parent);
...
find_property_index (MonoClass *klass, MonoProperty *property) {
	...
	for (i = 0; i < mono_class_get_ext (klass)->property.count; ++i) {
		if (property == &mono_class_get_ext (klass)->properties [i])
			return mono_class_get_ext (klass)->property.first + 1 + i;
	}
	return 0;
}
...
find_event_index (MonoClass *klass, MonoEvent *event) {
	...
	for (i = 0; i < mono_class_get_ext (klass)->event.count; ++i) {
		if (event == &mono_class_get_ext (klass)->events [i])
			return mono_class_get_ext (klass)->event.first + 1 + i;
	}
	return 0;
}
...
mono_reflection_create_generic_class (MonoReflectionTypeBuilder *tb)
{
	...
	if (mono_class_has_generic_container (klass) || (count == 0))
		return;

	g_assert (tb->generic_container && (tb->generic_container->owner.klass == klass));

	container = mono_image_alloc0 (klass->image, sizeof (MonoGenericContainer));
	klass->class_kind = MONO_CLASS_GTD;
	mono_class_set_generic_container (klass, container);

	container->owner.klass = klass;
	container->type_argc = count;
	container->type_params = mono_image_alloc0 (klass->image, sizeof (MonoGenericParamFull) * count);
	...
	for (i = 0; i < count; i++) {
		...
		container->type_params [i].info.pklass = NULL;
		container->type_params [i].info.flags = gparam->attrs;

		g_assert (container->type_params [i].param.owner);
	}

	container->context.class_inst = mono_get_shared_generic_inst (container);
}
...
ctorbuilder_to_mono_method (MonoClass *klass, MonoReflectionCtorBuilder* mb)
{
	...
	if (!((MonoDynamicImage*)(MonoDynamicImage*)klass->image)->save && !mono_class_has_generic_container (klass)) {
...
fix_partial_generic_class (MonoClass *klass)
{
	...
	dgclass = (MonoDynamicGenericClass *)  mono_class_get_generic_class (klass);
...
typebuilder_setup_fields (MonoClass *klass, MonoError *error)
{
	...
	mono_class_get_ext (klass)->field_def_values = image_g_new0 (image, MonoFieldDefaultValue, klass->field.count);
	/*
	...
	*/
	...
	for (i = 0; i < klass->field.count; ++i) {
		...
		if ((fb->attrs & FIELD_ATTRIBUTE_HAS_FIELD_RVA) && (rva_data = fb->rva_data)) {
			...
			mono_class_get_ext (klass)->field_def_values [i].data = data;
		}
		if (fb->offset != -1)
			field->offset = fb->offset;
		field->parent = klass;
		...
		if (fb->def_value) {
			...
			idx = encode_constant (assembly, fb->def_value, &mono_class_get_ext (klass)->field_def_values [i].def_type);
			/* Copy the data from the blob since it might get realloc-ed */
			p = assembly->blob.data + idx;
			len = mono_metadata_decode_blob_size (p, &p2);
			len += p2 - p;
			mono_class_get_ext (klass)->field_def_values [i].data = mono_image_alloc (image, len);
			memcpy ((gpointer)mono_class_get_ext (klass)->field_def_values [i].data, p, len);
		}
	}
...
typebuilder_setup_properties (MonoClass *klass, MonoError *error)
{
	...
	if (!mono_class_get_ext (klass))
		mono_class_set_ext (klass, image_g_new0 (image, MonoClassExt, 1));
	...
	for (i = 0; i < ext->property.count; ++i) {
		...
		if (pb->def_value) {
			...
			idx = encode_constant (assembly, pb->def_value, &ext->prop_def_values [i].def_type);
			/* Copy the data from the blob since it might get realloc-ed */
			p = assembly->blob.data + idx;
			len = mono_metadata_decode_blob_size (p, &p2);
			len += p2 - p;
			ext->prop_def_values [i].data = mono_image_alloc (image, len);
...
mono_reflection_create_runtime_class (MonoReflectionTypeBuilder *tb)
{
	...
	if (tb->subtypes) {
		for (i = 0; i < mono_array_length (tb->subtypes); ++i) {
			...
			mono_class_alloc_ext (klass);
			ext = mono_class_get_ext (klass);
			ext->nested_classes = g_list_prepend_image (klass->image, ext->nested_classes, mono_class_from_mono_type (mono_reflection_type_get_handle ((MonoReflectionType*)subtb)));
		}
	}
	 ...
	 */
	if (domain->type_hash && mono_class_has_generic_container (klass))
...
resolve_object (MonoImage *image, MonoObject *obj, MonoClass **handle_class, MonoGenericContext *context)
{
	...
	if (strcmp (obj->vtable->klass->name, "String") == 0) {
		...
		*handle_class = mono_defaults.fieldhandle_class;
		...
		if (fb->handle && mono_class_has_generic_container (fb->handle->parent)) {
