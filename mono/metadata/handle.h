...
FIXME This belong somewhere else.
*/
...
//FIXME respect the CHECKED_BUILD define
#define HANDLE_INVARIANTS(H) mono_handle_verify((void*)(H).__obj)

#define TYPED_HANDLE_NAME(__T) __T ## Handle
#define TYPED_HANDLE_DECL(__T, __SN) \
	typedef struct { __T **__obj; } TYPED_HANDLE_NAME (__T) ;
...
#define MONO_HANDLE_RAW(H) (HANDLE_INVARIANTS (H), (*(H).__obj))
...
#define MONO_HANDLE_SET(HANDLE, FIELD, VAL_HANDLE) do {	\
		MONO_OBJECT_SETREF (MONO_HANDLE_RAW (HANDLE), FIELD, MONO_HANDLE_RAW (VAL_HANDLE));	\
	} while (0);
...
#define MONO_HANDLE_ARRAY_SETREF(HANDLE, IDX, VALUE) do {	\
		int __idx = (IDX);	\
		mono_array_setref_fast (MONO_HANDLE_RAW (HANDLE), __idx, MONO_HANDLE_RAW (VALUE));	\
	} while (0)
...
TYPED_HANDLE_DECL (MonoString, string)
TYPED_HANDLE_DECL (MonoArray, array)
TYPED_HANDLE_DECL (MonoObject, object)
