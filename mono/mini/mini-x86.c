...
mono_x86_emit_tls2_get (guint8* code, int dreg, MonoFastTlsKey tls_key)
{
	...
	case MONO_FAST_TLS_MODEL_EMULATED_INDIRECT: {
		...
		x86_prefix (code, X86_GS_PREFIX);
		x86_mov_reg_mem (code, dreg, mono_tls_block_get_offset (), 4);
...
static guint8*
mono_x86_emit_tls2_set (guint8* code, int sreg, int scratch_reg, MonoFastTlsKey tls_key)
{
	int tls_offset = mono_tls_get_offset (tls_key);
#if defined(__APPLE__)
	switch (mono_tls_get_model ()) {
	...
	case MONO_FAST_TLS_MODEL_EMULATED_INDIRECT: {
		...
		x86_prefix (code, X86_GS_PREFIX);
		x86_mov_reg_mem (code, scratch_reg, mono_tls_block_get_offset (), 4);
...
static guint8*
mono_x86_emit_tls2_addr (guint8* code, int dreg, MonoFastTlsKey tls_key)
{
#if defined(__APPLE__)
	switch (mono_tls_get_model ()) {
	case MONO_FAST_TLS_MODEL_EMULATED_INDIRECT:
		x86_prefix (code, X86_GS_PREFIX);
		x86_mov_reg_mem (code, dreg, mono_tls_block_get_offset (), 4);
		x86_alu_reg_imm (code, X86_ADD, dreg, tls_key * 4 + G_STRUCT_OFFSET (MonoThreadInfo, tls_block));
		break;
	default:
		g_error ("Unsuported tls model %d", mono_tls_get_model ());
	}
...
#define EXTRA_CODE_SPACE (NACL_SIZE (16, 16 + kNaClAlignment))
		...
		case OP_TLS2_GET: {
			code = mono_x86_emit_tls2_get (code, ins->dreg, ins->inst_offset);
			break;
		}
		case OP_TLS2_SET: {
			code = mono_x86_emit_tls2_set (code, ins->sreg1, -1, ins->inst_offset);
			break;
		}
		case OP_TLS2_ADDR: {
			code = mono_x86_emit_tls2_addr (code, ins->dreg, ins->inst_offset);
			break;
		}
